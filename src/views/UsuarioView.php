<?php

namespace App\Views;

class UsuarioView extends BaseView
{
    private $usuarios;

    protected function getTitle()
    {
        return "Usuarios";
    }

    protected function getCurrentPage()
    {
        return "usuarios";
    }

    // El listado de usuarios no necesita script de cliente
    protected function getScript() {}

    public function setUsuarios($usuarios)
    {
        $this->usuarios = $usuarios;
    }

    protected function getContent()
    { ?>
        <div class="container">
            <div class="row">
                <table class="table table-striped">
                    <thead class=" sticky-top">
                        <tr class="text-center">
                            <th scope="col">Id</th>
                            <th scope="col">Nombre</th>
                            <th scope="col">Correo</th>
                            <th scope="col">Rol</th>
                            <th scope="col">Fecha Creación</th>
                            <th scope="col">Fecha Actualización</th>
                            <?php if ($this->isLogged && $this->rol == 'admin') { ?>
                                <th scope="col" colspan="2">Acciones</th>
                            <?php } ?>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($this->usuarios as $usuario) { ?>
                            <tr class="text-center">
                                <td><?= $usuario->getId() ?></td>
                                <td><?= htmlspecialchars($usuario->getNombre()) ?></td>
                                <td><?= htmlspecialchars($usuario->getCorreo()) ?></td>
                                <td><?= htmlspecialchars($usuario->getNombreRol()) ?></td>
                                <td><?= $usuario->getFechaCreacion() ?></td>
                                <td><?= $usuario->getFechaActualizacion() ?></td>
                                <?php if ($this->isLogged && $this->rol == 'admin') { ?>
                                    <td>
                                        <a href='actualizarUsuario.php?id=<?= $usuario->getId() ?>&con=<?= $usuario->getConcurrencia() ?>' class='btn btn-warning'>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                                <path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" />
                                                <path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" />
                                                <path d="M16 5l3 3" />
                                            </svg>
                                        </a>
                                    </td>
                                    <td>
                                        <a href='borrarUsuario.php?id=<?= $usuario->getId() ?>&con=<?= $usuario->getConcurrencia() ?>' class='btn btn-danger'>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-trash">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                                <path d="M4 7l16 0" />
                                                <path d="M10 11l0 6" />
                                                <path d="M14 11l0 6" />
                                                <path d="M5 7l1 12a2 2 0 0 0 2 2h8a2 2 0 0 0 2 -2l1 -12" />
                                                <path d="M9 7v-3a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v3" />
                                            </svg>
                                        </a>
                                    </td>
                                <?php } ?>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
            </div>
        </div>
<?php }
}
